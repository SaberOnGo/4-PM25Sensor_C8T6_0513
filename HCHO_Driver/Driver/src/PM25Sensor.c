...
static uint16_t PM25_AqiCalculate(uint16_t pm25, E_AQI_STD aqi_std, uint8_t *level)
{
    ...
    if(i < AQI_LEVEL_SIZE)	
    {
		...
		aqi = (uint16_t)(((double)deltaI) * (pm25 - c_low) / deltaC + AqiLevel[i].I_low);  // AQI 计算公式
    }
	else{ i = 0; }
...
static void LCD_Display_AQI(uint16_t pm25, E_AQI_STD aqi_std)
{
	...
	//if(is_rgb_on)LED_AqiIndicate((E_AQI_LEVEL)level);  // RGB 灯显示
...
void HCHO_CaliSet(void)
{
	...
	env.is_hcho_cal = 1;
	env.cali_mass = cali_mass;
	env.cali_ppb  = cali_ppb;
	F10X_FLASH_WriteSysEnv(&env);  // 记录用户设置
	#endif
}
...
static void TimerSensorTask_CallBack(void * arg)
{
   // static uint8_t is_rx_hcho_first_time = E_TRUE; // 是否第一次显示
	...
	if(rx_pm25)
    {
	   ...
	   {
	      PM25_Display_Init(DISPLAY_CC);
	      is_rx_pm25_first_time = E_FALSE;
	   }
       else
       {
          PM25_Display_Value(DISPLAY_CC);
       }
    }
    #endif      
	...
	{ 
		   ...
		   if(need_to_clear_screen)
		   {
				  ...
				  if(which_to_display == DIS_HCHO)
				  {
				     HCHO_DisplayInit(HCHO_DISPLAY_MASSFRACT);  // 甲醛显示初始化清屏
		   ...
		   else
		   {
			     if(which_to_display == DIS_HCHO)
			     {
			         HCHO_DisplayValue(HCHO_DISPLAY_MASSFRACT);
			     }
				 else
				 {
...
void PM25_Init(void)
{
   ...
   {
      is_cal = env.is_hcho_cal;
	  cali_mass = env.cali_mass;
	  cali_ppb  = env.cali_ppb;
   }
   else
   {
      is_cal = 1;
   ...
   os_timer_arm(&tTimerSwitchDisplay, 800, 0);  // 8 s 定时
   #endif
}
